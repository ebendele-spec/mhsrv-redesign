...
for path,heading,desc,intent in [
 ('sell-your-rv.html','Your next chapter starts here.','Ready to sell your RV or trade toward something new? Tell us about your RV and an MHSRV specialist will explain the next steps.','Discuss selling my RV'),
 ('trade-in.html','Make your current RV part of the plan.','Share your RV’s year, make, model, mileage and condition. A specialist will follow up about the appraisal process. A submitted request is not an offer.','Discuss my RV trade-in'),
 ('get-prequalified.html','Make room for your next adventure.','Explore financing with an MHSRV specialist. Ask about available lenders, the application process and what information you’ll need.','Discuss RV financing')]:
    extra='<p class="data-note">This contact form is not a credit application. Do not send Social Security numbers, bank details or other sensitive financial information. A specialist can direct you to the appropriate application process.</p>' if 'prequalified' in path else '<p class="data-note">Please do not submit account numbers or financial documents through the contact form. An appraisal or purchase offer requires a separate review.</p>'
    body=f'<main id="main" class="container content-page"><div class="page-heading"><p class="eyebrow">Motor Home Specialist</p><h1>{heading}</h1><p>{desc}</p></div><div class="contact-page-grid"><div class="article-body"><h2>Start with a conversation.</h2><p>Tell us what you have in mind, and include any stock numbers you’re considering. We’ll help you understand the details and your next step.</p>{extra}<button class="button yellow" data-lead="{intent}" style="margin-top:24px">Talk with a specialist {t.icon("arrow")}</button><p style="margin-top:22px">Prefer to call? <a href="tel:8003356054">800-335-6054</a></p></div><div class="contact-card"><h3>Helpful details to have ready</h3><p>Your preferred RV or stock number, the way you plan to use it, your purchase timeframe, and any trade-in details.</p><p style="margin-top:14px">You can ask questions before you decide. We’re here to help you make an informed choice.</p><a href="index.html#inventory">Explore the inventory {t.icon("arrow")}</a></div></div></main>'
    write(path,page(path,heading,desc,body))

about=f'<main id="main" class="container content-page"><div class="page-heading"><p class="eyebrow">Where the world shops</p><h1>Big possibilities.<br>Real people to help.</h1><p>Motor Home Specialist brings RV shopping, detailed information and personal guidance together. Visit us, compare your choices online, or ask for a walk-through before you travel.</p></div><img src="assets/showroom.jpg" alt="Motor Home Specialist in Alvarado, Texas" width="1200" height="675" style="width:100%;max-height:470px;object-fit:cover;border-radius:14px"><div class="contact-page-grid" style="margin-top:32px"><div class="article-body"><h2>Bring us your plans.</h2><p>Whether you’re considering your first RV or your next one, the important questions are yours: how you travel, what fits your family, what you want to bring and what you want to spend.</p><p>Explore the actual listing details, save your favorites and compare them. When you’re ready, our team can confirm availability, installed equipment, pricing and the next steps.</p><button class="button" data-lead="Plan a dealership visit" style="margin-top:20px">Plan your visit {t.icon("arrow")}</button></div><div><div class="contact-card"><h3>Alvarado, Texas</h3><p>5411 South I-35W<br>Alvarado, TX 76009</p><a href="tel:8003356054">800-335-6054</a></div><div class="contact-card"><h3>Palm Desert, California</h3><p>Contact our California team for current location information and appointments.</p><a href="tel:8335191129">833-519-1129</a></div><div class="contact-card"><h3>Montgomery, Alabama</h3><p>Contact our Alabama team for current inventory and appointments.</p><a href="tel:3342880331">334-288-0331</a></div></div></div></main>'
write('about.html',page('about.html','Visit Motor Home Specialist','Plan a visit or talk to the MHSRV teams in Texas, California and Alabama.',about))

raw=json.loads((ROOT/'mhsrv-reviews-all.json').read_text());stories=[]
for rec in raw:
    slug,title,stock,text,image,flag,ym=rec
    if not re.fullmatch(r'[a-zA-Z0-9_-]+',slug):raise ValueError('Invalid review slug')
    stories.append({'slug':slug,'title':title,'text':text.strip(),'date':ym})
    photo = '<img class="review-photo" src="'+t.E(image)+'" alt="Customer photo from the original review" loading="lazy" width="400" height="295">' if image.startswith('https://') else ''
    body=f'<main id="main" class="container narrow content-page"><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="../reviews.html">Customer stories</a><span>/</span><span>MHSRV customer experience</span></nav><div class="page-heading" style="padding-top:10px"><p class="eyebrow">An MHSRV customer story</p><h1>{t.E(title)}</h1><p>Published {t.E(ym)} · Historical customer testimonial</p></div>{photo}<blockquote class="review-content">{t.E(text).replace(chr(10),"<br>")}</blockquote><p class="data-note">This historical testimonial describes a customer’s experience and may refer to a previously sold RV. It does not establish the condition, equipment or availability of a current listing. No star rating has been inferred.</p><div class="inline-concierge"><div><h3>Start your own next chapter.</h3><p>Explore today’s imported listings and confirm availability with our team.</p></div><a href="../index.html#inventory" class="button">Find your RV {t.icon("arrow")}</a></div></main>'
    write('reviews/'+slug+'.html',page('reviews/'+slug+'.html',title,text[:155],body,'../'))
write('customer-stories.json',json.dumps(stories,ensure_ascii=False,separators=(',',':'))+'\n',False)
sample=''.join(f'<article class="review-card"><h3><a href="reviews/{r["slug"]}.html">{t.E(r["title"])}</a></h3><p>“{t.E(r["text"][:240])}{"…" if len(r["text"])>240 else ""}”</p><small>{t.E(r["date"])}</small><a class="text-link" href="reviews/{r["slug"]}.html">Read the story {t.icon("arrow")}</a></article>' for r in stories[:12])
body=f'<main id="main" class="container content-page"><div class="page-heading"><p class="eyebrow">The MHSRV community</p><h1>Real adventures.<br>In our customers’ words.</h1><p>Explore 4,571 historical customer stories from Motor Home Specialist. Search a brand, an RV model or a place to find experiences that interest you.</p></div><form class="search-form" id="reviewSearchForm" style="max-width:700px">{t.icon("search")}<input type="search" id="reviewSearch" aria-label="Search customer stories" placeholder="Search by brand, model or place"><button class="button">Search stories</button></form><p id="reviewCount" class="data-note">4,571 customer stories</p><div class="review-grid" id="reviewGrid">{sample}</div><div class="load-more-row"><button class="button outline" id="moreReviews">Read more stories {t.icon("arrow")}</button></div><p class="data-note">Testimonials describe the dealership experience and may discuss previously sold RVs. They are not reviews of every current stock number. Star ratings are not inferred from these records.</p></main>'
write('reviews.html',page('reviews.html','Customer Stories','Read thousands of historical Motor Home Specialist customer stories and search by brand, model or location.',body))
